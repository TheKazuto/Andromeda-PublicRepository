...
- **Zero-trust submit.** Between `prepare` and `submit`, the dWallet owner signs an off-chain challenge bound to the swap (`intents/swap/challenge`). Solana same-chain takes one signature; EVM two-step takes one bundle signature covering both approve + swap. The gateway cannot relay without it.
