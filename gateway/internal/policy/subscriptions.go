...
// by the PolicyEngine v3 admin flow; subsequent calls with the same
// `(api_key_id, policy_address)` are no-ops. The legacy table name
// (`policy_subscriptions`) is preserved by migration to avoid a destructive
// rewrite — the schema is identical between the legacy templates surface and
// the v3 PolicyEngine.
...
// `policy_address` (PolicyEngine v3 events) or `dwallet_address` (Ika
