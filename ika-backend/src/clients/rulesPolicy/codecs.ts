/**
 * Manual codecs for `contracts/rules-policy/src/lib.rs` (v2).
 *
 * Two account types:
 *  - `RulesPolicy` (discriminator = 1, 1208 bytes payload)
 *  - `QuorumSession` (discriminator = 2, 816 bytes payload)
 *
 * Both prefixed by an 8-byte discriminator slot (only the first byte carries
 * the value; trailing bytes are zero by Quasar convention).
 *
...
// Audit C2 (Opção 4): RulesPolicy now stores `init_authority_slot` for
// traceability and dashboard convenience. Account size grows by 34 bytes.
