...
func TestSolanaPrepareHappyPath(t *testing.T) {
	...
	adapter := newSolanaAdapter(nil, nil, "")

	out, err := adapter.Prepare(context.Background(), stepFor(b64, payer), lifi.QuoteParams{FromAddress: payer.String()})
	if err != nil {
...
func TestSolanaPrepareRejectsWrongFeePayer(t *testing.T) {
	...
	_, err := adapter.Prepare(context.Background(), stepFor(b64, payer), lifi.QuoteParams{FromAddress: other.String()})
	if err == nil {
...
func TestSolanaPrepareRejectsMissingTxRequest(t *testing.T) {
	...
	_, err := adapter.Prepare(context.Background(), step, lifi.QuoteParams{FromAddress: payer.String()})
	if err == nil {
